...
class CastleApiHelper {
  constructor(apiSecret) { this.apiSecret = apiSecret; }
  ...
  _getRequestData(req) {
    ...
    const ip = (req.get('x-forwarded-for') || req.connection.remoteAddress).split(',')[0];
  ...
  _createPayload(requestToken, ip, headers, userPayload) {
    return { ...userPayload, context: { ip, headers }, request_token: requestToken };
  }

  async _sendLoginEventToCastle(endpoint, payload) {
    return await axios.post('https://api.castle.io' + endpoint, payload,
      { auth: { password: this.apiSecret, username: '' } });
  }
}
